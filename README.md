...
## Execution
Clone the repository and use poetry to set up an environment. Execution of different operations are implemented in the `Makefile`.
Configurations and parameters associoated with each step are specified directly in the scripts `src/preparational.py` and `src/operational.py`.
